...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'moses.rijoe.net';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'kasbon-chat@indorich.com';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'kasbon1234';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'kasbon-chat@indorich.com';        // Masukkan default from pada email
